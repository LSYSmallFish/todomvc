			...
			<section class="main" v-show="todoList.length" v-cloak>
				...
				<ul class="todo-list">
					<li
						...
						class="todo"
						:key="todo.id"
						@click="changeActive(todo.id)"
						:class="{completed: todo.isComplete,editing: todo.isEdit}"
						v-show="allShow||(oneShow?todo.isComplete:!todo.isComplete)"
					>
						<div class="view">
						...
						<input
							...
							v-model="todo.text"
							v-show="todo.isEdit"
							v-todo-focus="todo.text"
							@blur="unEdit(todo)"
							@keyup.enter="changeTodoText(todo,$event.target)"
							@keyup.esc="cancelEdit($event.target)"
						/>
					</li>
				</ul>
			</section>
			<footer class="footer" v-show="todoList.length" v-cloak>
				<span class="todo-count">{{marks}} itmes left</span>
				<ul class="filters">
					<li>
						<a
							href="#"
...
export default {
	...
	methods: {
		addTodo(e) {
			...
			let isCompleteVal=false
			let isEditVal=false
			axios.post('/api/todo/addTodo',{
				id,val,isCompleteVal,isEditVal
			}).then(res=>{
				console.log(res);
			})
		...
		allComplete() {
			...
			if (this.remaining === 0) {
				this.todoList.map(todo => (todo.isComplete = true))
			} else {
				this.todoList.map(todo => (todo.isComplete = false))
				this.remaining = 0
			}
		},
		removeAll() {
